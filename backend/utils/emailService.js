...
const createTransporters = () => buildTransporterConfigs().map((config) => nodemailer.createTransport(config));
...
const sendContactEmails = async (contact) => {
    ...
    if (failedResults.length > 0) {
        const errorMessage = failedResults.map((result) => result.reason?.message || String(result.reason)).join('; ');
        return {
            sent: false,
            skipped: false,
